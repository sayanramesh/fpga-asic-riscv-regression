...
results_df = pd.read_csv('/home/sayan/dissertation/regression/predict_fpga/results_classify_reverse.csv')
...
def plot_summary_table(pdk, save_path):
    rows = []
    sub = results_df[results_df['pdk'] == pdk]
    for target in sub['target'].unique():
        ...
        rows.append({
            ...
            f'All-{n_feat[pdk]} Acc': f"{best_full['cv_accuracy_mean']:.3f}" if best_full is not None else 'n/a',
    ...
    ax.set_title(f'Best Combination + Classifier per FPGA Target '
                f'({pdk} -> FPGA, 3-band, chance = {CHANCE:.3f})',
                fontsize=12, pad=10)
    plt.tight_layout()
    save(save_path)
    plt.close()
...
def plot_model_ranking(save_path):
    fig, axes = plt.subplots(1, len(pdks), figsize=(8 * len(pdks), 4), squeeze=False)
    for ax, pdk in zip(axes[0], pdks):
        s = results_df[results_df['pdk'] == pdk]
        stats = s.groupby('model')['cv_accuracy_mean'].agg(['mean', 'median'])
        ...
        ax.set_xlabel(f'accuracy over all {s["combination"].nunique()} '
                      f'combinations x {s["target"].nunique()} targets')
        ax.set_ylabel('')
        ax.set_title(f'{pdk} -> FPGA classifier ranking', fontsize=11)
...
def plot_model_choice(save_path):
    ...
    for pdk in pdks:
        ...
        for target in pdk_df['target'].unique():
            ...
            for model, h in g.groupby('model'):
                full = h[h['combination'] == all_features[pdk]]
                if full.empty:
                    continue
            ...
            rows.append({
                ...
                'Target': label,
                'Model': model,
                f'All-{n_feat[pdk]} Acc': f"{a_full:.3f}",
    ...
    ax.set_title(f'Recommended classifier per FPGA target, chosen by accuracy '
                f'with all ASIC features (3-band, chance = {CHANCE:.3f})\n'
...
for pdk in pdks:
    plot_summary_table(pdk, f"{base}summary_table_classify_{pdk}")

plot_model_choice(f"{base}model_choice_classify_reverse")
plot_model_ranking(f"{base}model_ranking_classify_reverse")
